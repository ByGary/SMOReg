...
class AffinityInp(nn.Module):
    ...
    def forward(self, X, Y):
        assert X.shape[-1] == Y.shape[-1] == self.d
        M = torch.matmul(X, Y.transpose(0, 1))
        return self.sigm(M)


class AffinityPearson(nn.Module):
    def __init__(self, d):
        super().__init__()
        self.d = d
    ...
    def forward(self, X, Y):
        ...
        mean_X = torch.mean(X_expanded, dim=-1, keepdim=True)
        mean_Y = torch.mean(Y_expanded, dim=-1, keepdim=True)
        cov = torch.sum((X_expanded - mean_X) * (Y_expanded - mean_Y), dim=-1)
        std_X = torch.sqrt(torch.sum((X_expanded - mean_X) ** 2 + 1e-8, dim=-1))
        std_Y = torch.sqrt(torch.sum((Y_expanded - mean_Y) ** 2 + 1e-8, dim=-1))
...
class AffinityBiFC(nn.Module):
    ...
    def forward(self, X, Y):
        ...
        for i in range(self.bd):
            ...
            tmp = torch.matmul(tmp, Y.transpose(0, 1))
            bi_result[:, :, :, i] = tmp
        S = self.fc(bi_result)
